...
function chooseStencil(){var els=d.querySelectorAll('button,a,[role="button"]');for(var i=0;i<els.length;i++){var t=(els[i].textContent||'').trim().toLowerCase();if(t==='pochoir'||t.indexOf('pochoir')>=0){try{els[i].click();}catch(e){}return true;}}return false;}
function hideAdvanced(){for(var i=0;i<HIDE.length;i++){var el=d.getElementById(HIDE[i]);if(el)el.style.display='none';}}
function enforceSize(){var r=d.getElementById('sizeCm');if(r){r.min='2';if(parseFloat(r.value)<2)r.value='2';}var w=d.getElementById('s91w'),h=d.getElementById('s91h');if(w)w.min='2';if(h)h.min='2';}
function simpleBox(){
 ...
 for(var j=0;j<map.length;j++)(function(m){var ui=d.getElementById(m[0]),real=range(m[1]),val=d.getElementById(m[2]);if(!ui)return;ui.addEventListener('input',function(){if(real){real.value=ui.value;fire(real);}val.textContent=ui.value;});ui.addEventListener('change',function(){if(real){real.value=ui.value;fire(real);}val.textContent=ui.value;});})(map[j]);
 var btns=box.querySelectorAll('button');
 for(var k=0;k<btns.length;k++)btns[k].addEventListener('click',function(){var real=range(this.getAttribute('data-id'));if(!real)return;var step=parseFloat(real.step)||1,v=parseFloat(real.value)||0,min=parseFloat(real.min),max=parseFloat(real.max);v+=this.classList.contains('ss-minus')?-step:step;v=Math.max(min,Math.min(max,v));real.value=String(v);fire(real);var id=this.getAttribute('data-id');var ui=d.getElementById(id==='sDetail'?'ss-detail':id==='sReinforce'?'ss-reinforce':'ss-contrast');var out=d.getElementById(id==='sDetail'?'ssv-detail':id==='sReinforce'?'ssv-reinforce':'ssv-contrast');if(ui)ui.value=real.value;if(out)out.textContent=real.value;});
}
